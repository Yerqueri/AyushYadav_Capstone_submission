...
(e.g. sensitive data appearing in logs), or anything where the customer is reporting
a real problem but the service is still partially functional.
...
STEP 3 — CALIBRATE CONFIDENCE
0.75 — ongoing security concern (secrets in logs) inferred; no explicit production impact stated.
...
| 2. LOW screen | Hard gate (AND-logic) | NOT-LOW list explicitly blocks common false-LOW patterns (compliance, feature requests, security concerns, access revocations) |
...
| 3.2 | 2026-09-17 | STEP 1(b): added "deployed release actively causing errors" to HIGH examples; STEP 2(b): replaced broad NOT-LOW list with explicit categories — compliance/audit, feature requests, access revocation for departing employees, production access issues, production job/config failures, and ongoing security concerns in logs; restored single-user non-critical access as potentially LOW; added Example E (ongoing security concern) |

## Known Weaknesses (v3.2)

- Dataset contains label inconsistencies for `rollback_request` urgency (tickets with near-identical bodies labeled HIGH vs MEDIUM) — no prompt fix possible.
